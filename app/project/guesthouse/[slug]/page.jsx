...
const GRID = {
  gallery: 'grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 mt-8 w-full',
  highlights: 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8',
  videos: 'grid grid-cols-1 md:grid-cols-2 gap-6'
};

const STYLES = {
  title: 'text-3xl md:text-5xl font-bold text-center text-amber-800 mb-6',
  divider: 'w-36 h-1 bg-amber-700 mx-auto mb-8',
  button: 'bg-gradient-to-r from-amber-700 to-yellow-600 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded duration-300'
};
...
const VideoPlayer = React.memo(({ video }) => {
  ...
  if (error) return <div className='aspect-video bg-gray-200 flex items-center justify-center'>Video unavailable</div>;

  return (
    <div ref={ref} className='relative aspect-video rounded-lg overflow-hidden'>
      {show && (
        <video
          className='w-full h-full object-cover'
...
const GalleryImage = React.memo(({ item, onClick }) => {
  ...
  return (
    <div
      ...
      className={`relative aspect-video rounded-lg overflow-hidden ${error ? 'bg-gray-200' : 'cursor-pointer hover:scale-105 transition-transform duration-300'}`}
    >
      {!error ? (
        <>
      ...
      ) : <div className='flex items-center justify-center h-full text-sm text-gray-500'>Image unavailable</div>}
    </div>
  );
});
...
const HighlightItem = React.memo(({ item }) => {
  ...
  return (
    <div className='bg-white p-6 rounded-lg shadow-md flex flex-col items-center text-center hover:scale-105 transition-transform'>
      {Icon ? <Icon className='w-8 h-8 mb-4 text-cyan-600' /> : <div className='w-8 h-8 bg-gray-300 rounded mb-4' />}
      <p className='text-sm font-medium text-gray-900'>{item.description}</p>
    </div>
  );
});
...
const ConfigRow = React.memo(({ config }) => (
  <tr className='border-b hover:bg-gray-100'>
    ...
    <td className='py-3 px-4 text-center'>
      <Link href='/contact' className={STYLES.button}>Get Quote</Link>
    </td>
  </tr>
));
    ...
    <div className='bg-gradient-to-b from-white-100 to-white min-h-screen md:mt-36'>
      <section className='py-8 px-4 sm:px-6 lg:px-8'>
        <div className='flex flex-col lg:flex-row items-start gap-8'>
          <div className='lg:w-1/2'>
            <h1 className='text-2xl md:text-5xl font-bold text-amber-800 mb-4 text-center'>{project.title}</h1>
            <p className='text-sm md:text-base text-black text-center'>{project.fullDescription}</p>
          </div>
          <div className='lg:w-1/2'>
            <Image
              src={project.image}
      ...
      {(project.galleryImages || project.videos) && (
        <section className='bg-gray-100 py-12 px-4 sm:px-6 lg:px-8'>
          ...
          {project.videos?.length > 0 && <div className={GRID.videos}>{project.videos.map((v, i) => <VideoPlayer key={i} video={v} />)}</div>}
          {project.galleryImages?.length > 0 && <div className={GRID.gallery}>{project.galleryImages.map((img, i) => <GalleryImage key={i} item={img} onClick={setFullImage} />)}</div>}
        </section>
      )}
      ...
      {project.highlights?.length > 0 && (
        <section className='bg-gray-200 py-12 px-4 sm:px-6 lg:px-8'>
          ...
          <div className={STYLES.divider}></div>
          <div className={GRID.highlights}>{project.highlights.map((h, i) => <HighlightItem key={i} item={h} />)}</div>
        </section>
      )}
        ...
        <section className='bg-white py-12 px-4 sm:px-6 lg:px-8'>
          ...
          <div className='overflow-x-auto'>
            <table className='w-full border-collapse shadow-md rounded-lg'>
              ...
              <tbody>{project.configuration.map((c, i) => <ConfigRow key={i} config={c} />)}</tbody>
            </table>
          </div>
        </section>
      )}
